		...
		if (isAsync) {
			restHighLevelClient.bulkAsync(request, RequestOptions.DEFAULT, new ActionListener<BulkResponse>() {
				...
				public void onFailure(Exception e) {
					throw new IllegalStateException("Error occurred while bulk indexing operation: " + e.getMessage(), e);
				}
			});
		}
		else {
			try {
				...
				throw new IllegalStateException("Error occurred while bulk indexing documents", e);
			}
		}
	}
